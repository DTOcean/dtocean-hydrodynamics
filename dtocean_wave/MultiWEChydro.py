...
.. moduleauthor:: Francesco Ferri <ff@civil.aau.dk> and Pau Mercadez Ruiz <pmr@civil.aau.dk>
"""

from math import pi

from numpy import array, zeros, ones, transpose, dot, sqrt, arctan2, exp, tan, \
cos, sin, linspace, eye, reshape, meshgrid, real, imag, cosh, sinh, average
from numpy.linalg import solve
from scipy.special import jv, yv
import numpy as np

from .utils.WatWaves import len2, g, ro
...
class MultiBody(object):
    ...
    def __init__(self, ihydro, iwec, cylamplitude=True, debug=False):
        ...
        self.dir = ihydro.dir
        self.iwec = iwec
        (self.cpto, self.cfit, self.kmooring, self.kfit) = iwec.matrix_zoh_interp(ihydro)
    ...
    def energy(self, coord):
        ...
        Nm = array(range(-self.iwec.order.max(), self.iwec.order.max()+1), dtype=float)
        m, n = np.meshgrid(Nm, Nm, indexing = 'ij')
        Gp = np.transpose(self.iwec.G, axes = (0,2,1))
        wec_Fex = None
        wec_dir = None
        
        for orient, thetas in zip(self.dir[::2],self.dir[1::2]):
            ...
            (Pyr, P) = EnergyProduction(NBo, self.B, self.Hs, self.Tp,
                                        thetas, self.period, [self.ScatDiag,self.ScatDiag_spec],
                                        self.iwec.M, self.Madd, self.cpto,
                                        self.Crad, self.kmooring, self.iwec.Khyd, self.Fex,
                                        self.kfit, self.cfit)

            # Excitation Force isolated WEC
            (mths,mode) = meshgrid(ths, Nm, indexing = 'ij')
            AP = exp(-1j*mode*(pi/2+mths))
            Fex_iso = zeros((self.Fex.shape[:2]+(self.iwec.Fex.shape[2],)), dtype = complex)
            ...
            (PyrWEC, PWEC) = EnergyProduction(1, self.B, self.Hs, self.Tp,
                                        thetas, self.period, [self.ScatDiag,self.ScatDiag_spec],
                                        self.iwec.M, self.iwec.Madd, self.cpto,
                                        self.iwec.Crad, self.kmooring, self.iwec.Khyd, Fex_iso,
                                        self.kfit, self.cfit)
        ...
        device_model = {'wave_fr':1/self.period,'mode_def': translational_modes,'wave_dir': wec_dir, 'fex':Fex_for_mooring}
        machine = {}
        for jj in range(NBo):
            strn = 'Device%d'%jj
...
#            print 'period {} , depth {} '.format(requires[0],requires[1])
        ...
        self.Scattering(k0, direction, TS, DS_array, GS_array, MS_inter, iWEC.order[0])
        # Radiated waves and scattering between bodies
        self.Radiation(k0, cfreq, TR, DR_array, GR_array, MR_inter,
                       AR_rot, iWEC.Madd, iWEC.Crad, iWEC.order.max())
    ...
    def Interaction(self, k0, D, G, TruncOrder):
        ...
        T= zeros((len2(k0),Nb*dim,Nb*dim),dtype=complex)
        for i in range(len2(L)): # We will construct T[i,j] and T[j,i] at the same time so we just need len2(L)
            row= dim*ij0[i]
            col= dim*ij1[i]
            # " Assembling Tij and Tji "
            Tij= si*H[:,:,i][indp,abs(ind)]*exp(ind*alf[i]*1j)
            T[:,row:row+dim,col:col+dim]= Tij # Tij
            T[:,col:col+dim,row:row+dim]= Tij*exp(ind*pi*1j) # Tji
        ...
        D_array= zeros((len2(k0),Nb*dim,Nb*dim),dtype=complex)
        G_array= zeros((len2(k0),Nb*dim,Nb*dof),dtype=complex) # G is already transposed, numb columns= Nb*dof ok!
        for i in range(Nb):
            D_array[:,dim*i:dim*(i+1),dim*i:dim*(i+1)]= D # diffraction transfer matrix for the isolated device
        ...
        for i in range(len2(k0)):
            ...
            iS, iR= Nm-TruncOrder[i,0], Nm-TruncOrder[i,1]
            ...
            MS_inter.append(solve(eye(Nb*dimp[i,0])-dot(TS[i],DS_array[i]),eye(Nb*dimp[i,0])))
            MR_inter.append(solve(eye(Nb*dimp[i,1])-dot(TR[i],DR_array[i]),eye(Nb*dimp[i,1])))
            # " M_interaction=(Id-D*T)\Id=(Id-D*T)**-1 "
            
        return TS, TR, DS_array, DR_array, GS_array, GR_array, MS_inter, MR_inter
        
    def Scattering(self, k0, direction, T, D_array, G_array, M_interaction, BaseOrder):
        """
        ...
        matk0,matdir,X,mode= meshgrid(k0,direction,self.coord[:,0],array(range(-Nm,Nm+1)),indexing='ij')
        Y= meshgrid(k0,direction,self.coord[:,1],array(range(-Nm,Nm+1)),indexing='ij')[2]
        ...
        Fex= zeros((len2(k0),len2(direction),len2(G_array[0][0])),dtype=complex)
    ...
    def Radiation(self, k0, freq, T, D_array, G_array, M_interaction, AR_iso, Madd_iso, Crad_iso, BaseOrder):
        """
        ...
        for k in range(len2(k0)):
            ...
            # the entire array (i.e., solve system of equations, i.e. direct matrix method!!!) """
            Fex_rad= zeros((dof,Nb,dof*Nb),dtype=complex)
            # " Save amplitude coefficients "
            if self.cylamplitude:
                    self.AR.append(AR)
                    aRaux= zeros(AR.shape,dtype=complex)
            for i in range(dof):
                # """ ¤^t means transpose(¤)
            ...
            Madd_t= Madd_iso[k,:,:].transpose().repeat(Nb,axis=0).reshape((dof,1,dof*Nb))
            Crad_t= Crad_iso[k,:,:].transpose().repeat(Nb,axis=0).reshape((dof,1,dof*Nb)) 
        ...
        for i0 in range(NB):
            for i1 in range(NHs):
                for i2 in range(NTp):
                    ...
                    eP[ind_SS] = 1025.*9.81**2/64./np.pi*self.Hs[i1]**2*self.Tp[i2]*np.sqrt((max(self.coord[:,0])-min(self.coord[:,0]))**2+(max(self.coord[:,1])-min(self.coord[:,1]))**2)
